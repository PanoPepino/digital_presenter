...
from .movement import *  # Required to animate the creature within the load_script function
...
def load_script_tex(
        ...
        **kwargs):
    """This function merges together a list of actions for the creature and a list of text to synchronise with the aforementioned actions. It has an inner counter called counter_script.  
    ...
    # counts number of entries + 1. I assume last one to erase final text.
    for iteration in range(len(all_texts) + bool(fade_last)):
        if iteration < len(all_texts):
            type_de_script = [Create(all_texts[iteration],
                                     run_time=animation_rt), FadeIn(triangle_next_text, run_time=animation_rt), An_Cre(creature, all_actions[iteration], animation_rate_func=animation_rc, animation_run_time=animation_rt)]

        if iteration == 0:
            counter_script = type_de_script

        elif iteration < len(all_texts):
            counter_script = [FWC(all_texts[iteration-1], run_time=0.08), FadeOut(triangle_next_text, run_time=0.08), Create(all_texts[iteration], run_time=animation_rt), FadeIn(
                triangle_next_text, run_time=animation_rt), An_Cre(creature, all_actions[iteration], animation_rate_func=animation_rc, animation_run_time=animation_rt)]
